...
export default defineComponent({
  ...
  props: {
    ...
    // 是否关闭 label
    hideLabel: propTypes.bool.def(false),
    // 表单label宽度
  ...
  setup(props, { slots, expose, emit }) {
    ...
    // 监听formModel, 发送 change 事件
    watch(
      () => unref(formModel),
      (newModel) => {
        emit(
          'change',
          Object.entries(newModel).reduce((res, [key, value]) => {
            return {
              ...res,
              [key]: isProxy(value) || isReactive(value) || isRef(value) ? toRaw(value) : value
            }
          }, {})
        )
      },
    ...
    const renderFormItem = (item: FormSchema) => {
      ...
      return (
        <ElFormItem
          ...
          {...(item.formItemProps || {})}
          prop={item.field}
          label={props.hideLabel === false ? item.label : ''}
        >
          {formItemSlots}
        </ElFormItem>
      )
