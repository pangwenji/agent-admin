...
import {
  addSmartReduceMember,
  deleteSmartReduceUser,
  getSmartReduceMemberList
...
const { tableRegister, tableState, tableMethods } = useTable({
  ...
  fetchDataApi: async () => {
    ...
    const res = await getSmartReduceMemberList({
...
const handleAdd = async () => {
  ...
  if (account) {
    try {
      ...
      const res = await addSmartReduceMember({
        id: props.configId,
        lotteryCode: props.lotteryCode,
        account,
        smartType: props.smartType
  ...
  >
    <template #title>
      <ElSpace :size="16" class="mb-4">
        <ElInput v-model="memberAccount" placeholder="请输入会员账号" />
